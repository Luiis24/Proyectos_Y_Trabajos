...
function setgenero() {
  ...
  genero.forEach((generos) => {
    ...
    t.addEventListener("click", () => {
      ...
      const getMoviesURL = `https://api.themoviedb.org/3/discover/tv?sort_by=popularity.desc&api_key=3922568d95046025970a1b6575c3b3d7&language=es-MX&page=${pagina}`;
      getMovies(
  ...
  try {
    ...
    if (respuesta.status === 200) {
      ...
      datos.results.forEach((pelicula) => {
        peliculas += `
          <div class="card">
            <img id="${pelicula.id}" class="open-details-serie" src="https://image.tmdb.org/t/p/w500/${pelicula.poster_path}">
            <div class="titulo">
              <h4>${pelicula.original_name}</h4>
            </div>
          </div>
        `;
      });
...
const cargarpelicula = async () => {
  try {
    const respuesta = await fetch(
      `https://api.themoviedb.org/3/discover/tv?api_key=3922568d95046025970a1b6575c3b3d7&language=es-MX&page=${pagina}`
    );
    console.log(respuesta);
    if (respuesta.status === 200) {
      const datos = await respuesta.json();
